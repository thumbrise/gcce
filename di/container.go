...
// CompileStep описывает одну атомарную инструкцию по созданию компонента
type CompileStep struct {
	Index       int      `json:"index"`
	TargetType  string   `json:"target_type"`  // Например: "*cmds.StubSubCMD"
	PackagePath string   `json:"package_path"` // Например: "://github.com"
	FuncName    string   `json:"func_name"`    // Например: "NewStubSubCMD"
	ReturnsErr  bool     `json:"returns_err"`  // Нам нужно знать, генерить ли "if err != nil"
	ArgsTypes   []string `json:"args_types"`   // Список типов, которые этот конструктор ждет на вход
}

func (c *Container) Compile(invocation Invocation, options ...InvokeOption) error {
	err := c.compile(invocation, options...)
	if err != nil && knownError(err) {
		return errWithStack(err)
	}
	if err != nil {
...
// compile (Твоя главная точка перехвата)
func (c *Container) compile(invocation Invocation, _ ...InvokeOption) error {
	if invocation == nil {
		return fmt.Errorf("%w, got %s", errInvalidInvocationSignature, "nil")
	}
	fn, valid := inspectFunction(invocation)
	...
	// 1. Запускаем сбор шагов в глубину (DFS)
	var steps []CompileStep
	visited := make(map[reflect.Type]bool)

	for _, n := range nodes {
		steps, err = c.gsgWalk(n, steps, visited)
		if err != nil {
			return err
		}
	}

	// 2. РАЗВОРАЧИВАЕМ СЛАЙС (Bottom-Up экономика)
	// DFS шел от корня к листьям. Нам нужно от листьев к корню,
	// чтобы сначала создались зависимости, а потом передались в конструкторы выше.
	orderedSteps := make([]CompileStep, len(steps))
	for i, step := range steps {
		step.Index = i
		orderedSteps[len(steps)-1-i] = step
	}
	// Пересчитываем правильные индексы после разворота
	for i := range orderedSteps {
		orderedSteps[i].Index = i
	}

	// 3. Выплевываем чистейший, готовый GSG JSON
	j, err := json.MarshalIndent(orderedSteps, "", "  ")
	if err != nil {
		return fmt.Errorf("cannot marshal gsg plan: %w", err)
	}

	fmt.Printf("=== ТВОЙ ИДЕАЛЬНЫЙ GSG EXECUTION PLAN ===\n%s\n=========================================\n", string(j))

	return nil
}

// gsgWalk собирает метаданные конструкторов, проваливаясь вглубь по графу
func (c *Container) gsgWalk(n *node, steps []CompileStep, visited map[reflect.Type]bool) ([]CompileStep, error) {
	if n == nil {
		return steps, nil
	}
	...
	// Если узел собран через конструктор функций — выковыриваем его кишки
	if ctor, ok := n.compiler.(*constructorCompiler); ok {
		...
		// Парсим полное имя функции на пакет и имя
		...
		// Вытаскиваем типы всех входящих аргументов из встроенного reflect.Type функции!
		for i := 0; i < ctor.fn.NumIn(); i++ {
			step.ArgsTypes = append(step.ArgsTypes, ctor.fn.In(i).String())
		}

		// Сначала добавляем текущий шаг
		steps = append(steps, step)
	}

	// Проваливаемся рекурсивно в зависимости (к детям)
	childNodes, err := n.deps(c.schema)
	if err != nil {
		return nil, fmt.Errorf("cannot get deps for %s: %w", n.rt.String(), err)
	}

	for _, child := range childNodes {
		steps, err = c.gsgWalk(child, steps, visited)
		if err != nil {
			return nil, err
		}
	}
